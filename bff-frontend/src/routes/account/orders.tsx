  ...
  return (
    <main className="min-h-screen bg-background px-4 pb-16 pt-28 text-frost-white sm:px-8">
      <div className="mx-auto max-w-5xl">
        <Link to="/" className="mb-8 inline-flex items-center gap-2 text-sm text-steel-silver hover:text-ice-blue">
          <ArrowLeft className="h-4 w-4" /> Back to catalog
        </Link>
        <div className="mb-8 flex items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-ice-blue">Customer account</p>
        ...
        {!isLoading && !error && orders.length === 0 && (
          <div className="rounded-2xl border border-white/10 bg-white/5 px-6 py-14 text-center">
            ...
            <Link to="/products" className="mt-6 inline-flex rounded-full bg-gradient-primary-cta px-5 py-2.5 text-sm font-semibold text-white">Browse products</Link>
          </div>
        )}
        {!isLoading && !error && orders.length > 0 && (
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
            ...
            {orders.map((order) => (
              <Link key={order.id} to="/account/orders/$id" params={{ id: order.id }} className="grid gap-2 border-b border-white/10 px-5 py-4 last:border-b-0 hover:bg-white/5 sm:grid-cols-[1.3fr_1fr_1fr_1fr] sm:items-center sm:gap-4">
                <span className="font-mono text-sm text-ice-blue">{order.order_code || order.id}</span>
                <span className="text-sm text-steel-silver">{new Date(order.order_date || '').toLocaleDateString()}</span>
                <span className="text-sm">{order.currency} {order.total_amount}</span>
                <span className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-ice-blue/10 px-2 py-1 text-ice-blue">{order.payment_status}</span><span className="rounded-full bg-white/10 px-2 py-1 text-steel-silver">{order.fulfillment_status}</span></span>
              </Link>
            ))}
          </div>
        )}
